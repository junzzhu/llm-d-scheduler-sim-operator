...
func (s *StreamingServer) Process(srv extProcPb.ExternalProcessor_ProcessServer) error {
	...
	for {
		...
		if recvErr == io.EOF || status.Code(recvErr) == codes.Canceled {
			logger.V(logutil.DEBUG).Info("ext_proc stream closed", "error", recvErr)
			return nil
		}
		if recvErr != nil {
		...
		logger.Info("ext_proc request received", "type", fmt.Sprintf("%T", req.Request))
		...
		case *extProcPb.ProcessingRequest_RequestHeaders:
			...
			logger.Info("ext_proc request headers received", "request_id", requestID)
			logRequestHeaders(logger, v)
			reqCtx.sawRequestHeaders = true
			err = s.HandleRequestHeaders(reqCtx, v)
		case *extProcPb.ProcessingRequest_RequestBody:
			loggerTrace.Info("Incoming body chunk", "EoS", v.RequestBody.EndOfStream)
			logger.Info("ext_proc request body chunk received", "end_of_stream", v.RequestBody.EndOfStream, "body_bytes", len(v.RequestBody.Body))
			...
			if v.RequestBody.EndOfStream {
				...
				if errUnmarshal := json.Unmarshal(body, &reqCtx.Request.Body); errUnmarshal != nil {
					if logger.V(logutil.DEBUG).Enabled() {
						logger.Info("Error unmarshaling request body", "body", bodySnippet(body), "err", errUnmarshal)
					}
					err = errutil.Error{
				...
				logger.V(logutil.DEBUG).Info(
					"ext_proc generated request responses",
					"saw_request_headers", reqCtx.sawRequestHeaders,
					"request_state", reqCtx.RequestState,
					"req_body_responses", len(reqCtx.reqBodyResp),
					"request_size", reqCtx.RequestSize,
				)
		...
		case *extProcPb.ProcessingRequest_ResponseHeaders:
			...
			if statusVal != "" || contentType != "" {
				logger.V(logutil.DEBUG).Info("ext_proc response headers summary", "status", statusVal, "content_type", contentType)
			}
		...
		loggerTrace.Info("checking", "request state", reqCtx.RequestState, "saw_request_headers", reqCtx.sawRequestHeaders)
...
func (r *RequestContext) updateStateAndSendIfNeeded(srv extProcPb.ExternalProcessor_ProcessServer, logger logr.Logger) error {
	...
	if r.RequestState == RequestReceived && r.reqHeaderResp != nil && r.sawRequestHeaders {
		logger.V(logutil.DEBUG).Info("ext_proc send request header response", "state", r.RequestState)
	...
	if (r.RequestState == HeaderRequestResponseComplete || (r.RequestState == RequestReceived && !r.sawRequestHeaders)) && r.reqBodyResp != nil && len(r.reqBodyResp) > 0 {
		logger.V(logutil.DEBUG).Info("ext_proc send request body responses", "count", len(r.reqBodyResp))
	...
	if r.RequestState == ResponseReceived && r.respHeaderResp != nil {
		logger.V(logutil.DEBUG).Info("ext_proc send response header response", "state", r.RequestState)
	...
	if r.RequestState == HeaderResponseResponseComplete && r.respBodyResp != nil && len(r.respBodyResp) > 0 {
		logger.V(logutil.DEBUG).Info("ext_proc send response body responses", "count", len(r.respBodyResp))
...
func logRequestHeaders(logger logr.Logger, v *extProcPb.ProcessingRequest_RequestHeaders) {
	if v == nil || v.RequestHeaders == nil || v.RequestHeaders.Headers == nil {
		logger.V(logutil.DEBUG).Info("ext_proc request headers missing")
		return
	}
	headersCount := len(v.RequestHeaders.Headers.Headers)
	method := requtil.ExtractHeaderValue(v, ":method")
	path := requtil.ExtractHeaderValue(v, ":path")
	authority := requtil.ExtractHeaderValue(v, ":authority")
	contentType := requtil.ExtractHeaderValue(v, "content-type")
	contentLength := requtil.ExtractHeaderValue(v, "content-length")
	logger.V(logutil.DEBUG).Info(
		"ext_proc request headers summary",
		"method", method,
		"path", path,
		"authority", authority,
